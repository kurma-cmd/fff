...
import { getUniversityTier, isIvyLeague, universityTierInfo } from '../lib/universityTiers';
...
export function UniversityCard({ university, profile, onStudyLanguage, saved, onToggleSaved }: Props) {
  ...
  return <article className="university-card">
    <div className="uni-heading"><div className="uni-logo">🎓</div><div><div className="uni-tier-line"><span className={`uni-tier tier-${tier}`}>{universityTierInfo[tier].label}</span>{isIvyLeague(university) && <span className="ivy-badge">Ivy League</span>}</div><h3>{university.name}</h3><p>{university.city} · {university.country}</p></div><span className={`match-badge match-${match.score}`}>{match.label}</span></div>
    ...
    <div className="verification-note">Подборка пересчитана по твоему текущему профилю · Перед подачей обязательно сверь требования выбранной программы</div>
    </div>}
